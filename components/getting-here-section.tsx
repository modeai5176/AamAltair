...
import { Train, Car, MapPin, Shield, Wifi, Check, ExternalLink, Clock, Navigation } from "lucide-react"
...
const journeyData = {
  train: {
    origins: [
      {
        label: "From Nairobi",
        station_primary: { name: "Kibwezi SGR", rail_time: "3.5h", pickup_time: "1h" },
        station_alt: { name: "Makindu SGR", rail_time: "4h", pickup_time: "1.5h" }
      },
      {
        label: "From Mombasa", 
        station_primary: { name: "Kibwezi SGR", rail_time: "2h", pickup_time: "1h" }
      },
      {
        label: "From Nairobi (Makindu)",
        station_primary: { name: "Makindu SGR", rail_time: "4h", pickup_time: "1.5h" }
      }
    ],
...
const safetyNotes = [
  {
    iconName: "Shield",
    title: "Security",
    description: "Solar electric fence deters resident game. Secure on-site parking available.",
  },
  {
    iconName: "Wifi",
    title: "Connectivity",
    description: "Wi-Fi available at main house. Patchy mobile reception in some areas.",
  },
  {
    iconName: "MapPin",
    title: "Location",
    description: "GPS coordinates provided upon booking confirmation for accurate navigation.",
  },
]
    ...
    <section className="py-20 px-6 bg-background">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-serif font-medium text-primary mb-6">Getting Here</h2>
          <p className="text-xl text-foreground/80 max-w-2xl mx-auto text-pretty">
            Close to the wild, reachable by road or rail. Your journey to Aam Altair is part of the adventure.
          </p>
        </div>
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 mb-6">
                        <div className="flex items-center space-x-2">
                          <span className="text-lg font-medium text-primary">
                            {journeyData.train.origins[selectedOrigin].label.split(' ')[1]}
                          </span>
                          <span className="text-foreground/60">→</span>
                          <span className="text-lg font-medium text-primary">
                            {journeyData.train.origins[selectedOrigin].station_primary.name}
                          </span>
                        </div>
                        <Badge className="bg-accent/20 text-accent border-accent">
                          {journeyData.train.origins[selectedOrigin].station_primary.rail_time}
                        </Badge>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-foreground">Book SGR to {journeyData.train.origins[selectedOrigin].station_primary.name}</span>
                        </div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-foreground">
                            Arrive → Pickup {journeyData.train.origins[selectedOrigin].station_primary.pickup_time} to Aam Altair
                          </span>
                        </div>
                    ...
                    <div className="flex flex-col items-end space-y-3">
                      <div className="text-right">
                        <div className="text-lg font-medium text-accent">{journeyData.train.cta.price}</div>
            ...
            <CardContent className="p-8">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 mb-6">
                    <div className="flex items-center space-x-2">
                      <span className="text-lg font-medium text-primary">{journeyData.drive.route.origin}</span>
                      <span className="text-foreground/60">→</span>
                      <span className="text-lg font-medium text-primary">{journeyData.drive.route.destination}</span>
                    </div>
                    <Badge className="bg-accent/20 text-accent border-accent">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          {safetyNotes.map((note) => {
            const IconComponent = iconMap[note.iconName as keyof typeof iconMap]
            return (
              <Card key={note.title} className="bg-card border-border">
                <CardContent className="p-6 text-center space-y-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="font-serif text-lg font-medium text-primary">{note.title}</h3>
                    <p className="text-sm text-foreground/80 leading-relaxed">{note.description}</p>
                  </div>
                </CardContent>
              </Card>
            )
